 ...
 * 本地更新（源码仓库）按固定顺序执行：
 *
 *   preflight（基线 HEAD + 工作区干净度）→ git pull --ff-only → 重放本地补丁
...
async function stepPatches(repoRoot) {
  ...
  for (const file of patchState.files) {
    ...
    if (reverse.status === 'ok') {
      const entry = { name: patchName, status: 'upstreamed', code: 0, message: '已上游化', paths: touched };
      record.patches.push(entry);
      updateRun.patches.push(entry);
      record.lines.push(`  ${patchName}：已上游化（reverse --check 通过），跳过`);
      continue;
    }
...
/** 步骤 5：corepack pnpm run build（唯一规范构建入口）。 */
